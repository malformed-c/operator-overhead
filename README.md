...
⚠ ***THIS COLUMN DOES NOT MEASURE WHAT THIS FILE SAID IT MEASURED, AND THE NUMBER IS WITHDRAWN.*** It read: *waking a parked program through a shared wake index costs more than an informer callback in the same process*. Measured afterwards on the host that produced the ladder: **the wake index delivered nothing, for any kind, for eleven hours.** Radiant tags every wake with its own cause, and in that process all **1006** of them read `backstop elapsed (via poll, NOT the index)` and **none** read `condition became true (via the wake index)`. Arm B was paced by the poll re-evaluating its parked resume — the same clock as the yield path — so 37 ms is a sample of poll-paced convergence and not of a wake.

**An earlier version of this paragraph said the fault was ConfigMap-specific: 0 wakes for ConfigMap-parked programs against 1005 for Deployment-parked ones. That difference is not real.** Both figures counted `Woke` events without reading their attribution, and every one of the 1005 contains the words *NOT the index* in the same string being counted. What that comparison actually measured was **backstop frequency** — how often each program's deadline elapsed — and no kind was being woken by the index at all.
...
- **The 15.7 ms reaction is a wake-regime figure, and the regime has a boundary this ladder walks toward.** A Perseid's wake subscription exists *only while it is parked* — taken when the park begins, cancelled when it returns — so a change arriving mid-pass has no subscriber, and is instead seen at the next park by the `Holds` check the loop runs **before its first tick**, at zero wait. Nothing is lost. But those convergences are found by re-parking rather than by a wake, and their latency is governed by **pass duration**, not by the index. As the change rate or the pass duration rises, a growing share of convergences are re-park convergences, and a reaction figure that does not separate the two silently averages a mechanism it is measuring with one it is not. At 1 Hz against a millisecond step this run is almost entirely in the wake regime; at a higher rate or with a slower step it would not be, and nothing in the output would say so. Read off radiant's park loop, not measured here.
